...
def test_validation_result():
    r = ValidationResult("bogus-flow.json")
    ...
    assert str(r.failures[0]) == "[warning] my warning"

    r.add_error("my error")
    assert r.success == False
    assert r.strict_success == False
    assert str(r.failures[1]) == "[error] my error"
...
def test_validation_result_exc():
    ...
    exc = Exception("my exc")
    r.add_exc(exc)
    assert r.success == False
    assert r.strict_success == False
    assert str(r.failures[0]) == "[error] my exc"
    assert r.failures[0].exc is exc
...
#                 "modified": "2022-08-02T19:34:35.143Z",
#                 "operator": "AND",
#                 "effect_refs": ["attack-action--d68e5201-796c-469c-b012-290b7040db02"],
#                 "extensions": {
#                     "extension-definition--fb9c968a-745b-4ade-9b25-c324172197f4": {
#                         "extension_type": "new-sdo"
#                     }
...
#     with SCHEMA_PATH.open() as schema_file, NamedTemporaryFile("w+") as doc1_file:
