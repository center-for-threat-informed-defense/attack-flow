...
def generate_schema_docs(schema_doc, old_docs):
    '''
...
def insert_html(old_docs, html):
    '''
    Scan through text in ``old_docs`` and replace the contents of
    the <!--JSON_SCHEMA--> tags with new text from ``html``.
    '''
    output = ''
    old_docs_lines = old_docs.split('\n')

    # Output up to and including end tag.
    for line in old_docs_lines:
        output += line
