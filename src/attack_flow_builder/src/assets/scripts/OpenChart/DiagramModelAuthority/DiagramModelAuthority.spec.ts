...
import { Block, Group, Latch, Line } from "../DiagramModel";
import { DiagramModelAuthority } from "./DiagramModelAuthority";
import { DiagramObjectFactory, DiagramObjectType, PropertyType } from "./DiagramSchema";
import type { DiagramExport } from "./DiagramSerializer";
import type { DiagramSchemaConfiguration } from "./DiagramSchema";
...
export const sampleExport: DiagramExport = {
...
const authority = new DiagramModelAuthority();
...
describe("DiagramModelAuthority", () => {
    describe("Schema Registration", () => {
        it("registers sample schema", () => {
            const factory = authority.resolveEquivalentFactory(sampleSchema.id);
            expect(factory).toBeInstanceOf(DiagramObjectFactory);
        });
        it("creates valid page from schema", () => {
            const file = authority.createEmptyFile(sampleSchema.id);
            expect(file).toBeInstanceOf(Group);
        });
    ...
    describe("Diagram Imports", () => {
        it("imports valid export", () => {
            const objects = authority.importObjects(sampleExport);
