...
import { GroupCommand, SetStringProperty } from "@/stores/Commands/PageCommands";
...
class AttackFlowProcessor extends DiagramProcessor {
    ...
    private autofillFromTactic(command: SetStringProperty): PageCommand[] {
        ...
        if (value !== null && AttackFlowProcessor.TacticRecs.has(value)) {
            ...
            // Resolve tactic ID
            const tacticId = /TA[0-9]{4}/i.exec(value);
            if (tacticId === null) {
            ...
            // Resolve tactic
            const tactic = intel.tactics.find(o => o.id === tacticId![0])!;
            if (!tactic) {
                throw new Error(`Could not resolve tactic '${tacticId}'.`);
            }

            // Autofill
            const rootProps = property.root!.value;
            const nameProp = rootProps.get("name")! as StringProperty;
            const tacticRefProp = rootProps.get("tactic_ref")! as StringProperty;

            // Autofill name ONLY if empty or matches another autofilled tactic name
            const nameValue = nameProp.toRawValue();
    ...
    private autofillFromTechnique(command: SetStringProperty) {
        ...
        if (value !== null && AttackFlowProcessor.TechniqueRecs.has(value)) {
            ...
            const techniqueId = /T[0-9]{4}(?:\.[0-9]{3})?/i.exec(value);
            if (techniqueId === null) {
            ...
            // Resolve technique
            const technique = intel.technique.find(o => o.id === techniqueId![0])!;
            if (!technique) {
            ...
            const rootProps = property.root!.value;
            const nameProp = rootProps.get("name")! as StringProperty;
            const techniqueRefProp = rootProps.get("technique_ref")! as StringProperty;
