import { GroupCommand, HideSplashMenu, LoadFile, NullCommand } from "./";
...
export class PrepareEditorWithFile extends GroupCommand {
     ...
     *  A Promise that resolves with the {@link PrepareEditorWithFile} command.
     */
    public static async fromFileSystem(context: ApplicationStore): Promise<PrepareEditorWithFile | NullCommand> {
        const loadFile = await LoadFile.fromFileSystem(context);
        if (loadFile instanceof NullCommand) {
            return loadFile;
        } else {
            return new PrepareEditorWithFile(context, loadFile);
        }
    }
