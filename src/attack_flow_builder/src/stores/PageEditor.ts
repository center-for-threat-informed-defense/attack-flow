...
import {
    ...
    PageModel,
    type CameraLocation,
    type BlockDiagramSchema,
    type DiagramObjectExport
...
export class PageEditor {
    ...
    public redo() {
        if (this._redoStack.length) {
            const index = this._redoStack.length - 1;
            this._redoStack[index]!.execute();
    ...
    public static async createNew(schema: BlockDiagramSchema): Promise<PageEditor> {
        const factory = await DiagramFactory.create(schema);
        const page = factory.createObject(schema.page_template) as PageModel;
    ...
    public static async fromFile(file: string): Promise<PageEditor> {
        ...
        // Parse document
        const page = JSON.parse(file) as PageExport;

        // Initialize diagram factory
        const factory = await DiagramFactory.create(Configuration.schema);
