...
.. ATTACK_FLOW_SCHEMA Generated by `af` tool at 2022-08-15T16:06:08.591347Z
...
.. list-table::
   ...
   * - **scope** *(required)*
     ...
     - Indicates what type of behavior the Attack Flow describes: a specific incident,
       a campaign, etc.
   * - **start_refs** *(required)*
     - ``list`` of ``identifier`` (of type ``attack-action`` or ``attack-condition``)
      ...
      "external_references": [
        {
          ...
          "url": "http://ficitious.company.com/apt-x-campaign-report/"
        },
        {
          ...
          "url": "http://imaginary.company.com/apt-x-threat-actor/"
        }
      ],
      "extensions": {
...
.. list-table::
   ...
   * - **effect_refs** *(optional)*
     - ``list`` of ``identifier`` (of type ``attack-action`` or ``attack-operator`` or ``attack-condition``)
...
.. list-table::
   ...
   * - **type**
     - ``string``
     - The value of this property **must** be ``attack-condition``.
